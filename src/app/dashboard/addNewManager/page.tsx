...
const AddNewManager = () => {
  ...
  const uploadImageToBackend = async (image: any) => {
    ...
    try {
      const response = await fetch(
        `https://api.imgbb.com/1/upload?key=498c13144329f4ea75fda2875c5782b9`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleSubmit = async (e: any) => {
    ...
    const drivingLicenseExpirationDate =
      form.drivingLicenseExpirationDate?.value || "";
      ...
      <>
        <div className="w-full">
          <form
            ...
            className="container mx-auto my-[50px]  round-[16px] p-[50px]  shadow-[0 0 20px rgba(89, 102, 122, .05)] "
          >
            <h2 className="font-bold text-center text-[40px] my-[20px]">
              Add a new Manager
            </h2>
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="w-full col-span-12 add_driver">
                <textarea
                  required
                  className="w-full  border rounded-[5px] border-[#dee2e6]"
                  name="about"
                  id="about"
                  placeholder="About...."
                  cols={numCols}
            ...
            <div className="text-center mt-[15px]">
              <button type="submit" className="common_button">
                {isLoading ? <>Loading...</> : <>Add New Manager</>}
              </button>
            </div>
          </form>
        </div>
